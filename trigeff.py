...
"""
import numpy as np
from analib import inc, Hist, PhysObj, Event, dphi, dframe as DataFrame
import pandas as pd
import copy as cp
...
def loadjets(jets, events,bgweights=False):
    ...
    jets.event = DataFrame(events.array('event')).rename(columns=inc)
    jets.npvs  = DataFrame(events.array('PV_npvs')).rename(columns=inc)
    jets.npvsG = DataFrame(events.array('PV_npvsGood')).rename(columns=inc)
    ...
    if bgweights:
        jets.HT = DataFrame(events.array('LHE_HT')).rename(columns=inc)
    ...
    if "C" in stage or stage == "AB":
        for i in range(len(files)):
            ...
            if sjets[i].pt.shape[0] < 1 or jets[i].pt.shape[0] < 1:
                continue
            sjets[i].fixcols(first=1)
            jets[i].fixcols(first=1)
...
def overlay(stage):
    ...
    if len(stage) > 1:
        ...
        qcd,dt,rt = {},{},{}
        for st in starr:
            qcd.update({st: pickle.load(open(f"Seffplots/Combined QCD_{st}_Parked_ScaleFactor.p",'rb'))})
        ...
        tt = {"meta":{"AB":rt["AB"]["pt"][1],"ABC":rt["ABC"]["pt"][1],"CX":rt["CX"]["s2deepb"][1]},
              ...
              "ABser":rt["AB"]["pt"].ser,"ABCser":rt["ABC"]["pt"].ser,"CXser":rt["CX"]["s2deepb"].ser}
        pickle.dump(tt, open(f"TrigTensor2.p",'wb'))
        sys.exit()
