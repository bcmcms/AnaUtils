...
def ana(sigfile,bgfile,LOADMODEL=True,TUTOR=False,passplots=False,NET="F",subf=''):
    ...
    pplots = {
        ...
        "CSVV2":    Hist(40, (0.6,1)    ,'CSVV2 for highest pT jet in all (red), passing signal (blue), and signal (black) events','Fractional Distribution','netplots/pCSVV2'),
        ...
        "n2b1":     Hist(25 ,(0,0.5)    ,'n2b1 for highest pT jet','Fractional Distribution','netplots/pn2b1'),
    ...
    vplots = {
        ...
        "eta":      Hist(30 ,(0,3)     ,'|eta| for highest pT jet','Fractional Distribution','netplots/eta'),
        ...
        "CSVV2":    Hist(40, (0.6,1)    ,'CSVV2 for highest pT jet','Fractional Distribution','netplots/CSVV2'),
        ...
        "n2b1":     Hist(25 ,(0,0.5)    ,'n2b1 for highest pT jet','Fractional Distribution','netplots/n2b1'),
...
#           plots['genAmass'].dfill(As.mass)
        ...
        #     if 'sig' in jets.name:
        #         print('mass:',sigjets.pt.shape)
        #     jets.cut(jets.DeepB > 0.4184)
        #     if 'sig' in jets.name:
        #         print('DeepB:',sigjets.pt.shape)
        #     jets.cut(jets.DDBvL > 0.8)
        #     if 'sig' in jets.name:
        #         print('DDBvL:',sigjets.pt.shape)
        ...
        for jets in bgjets + [sigjets]:
            jets.cut(jets.pt.rank(axis=1,method='first',ascending=False) == 1,drop=False)
            for prop in jets:
        ...
        # Training Neural Net #
        #######################
        print('Average Weight:',sigevv.extweight.sum()/sigevv.extweight.shape[0])
...
def dialogue():
    print("Expected:\n mndwrm.py -s [Signal.json] -b [-Background.json]")
    print("---formatting flags--")
    print("-s __  Marks the signal .json file (can be data)")
    print("-b __  Marks the background .json file (can be data)")
    print("-f __  Saves the results to a specified subdirectory")
    print("-Pass  Indicates pass/fail plots should be made instead of regular plots (depreciated)")
    ...
    print("-A/B/C Selects a subnet to use instead of full network F")
