...
###################
    ...
    if (len(sys.argv) > 1):     ## Allows for files to be given as arguments
...
#############
    ...
    plots = {
        ...
        "h_met": histo('h_met', 'MET; ', [200,0,200]),
        ...
        for jEvt in range(ch.GetEntries()):
            ...
            for iMu in range(nMuons):
                ...
                for i in range(len(muEta)):
                    ...
                    if (abs(mu_eta - muEta[i]) < muEta[i]*.2) and (abs(mu_pt - muPt[i]) < 0.1) and (abs(mu_phi - muPhi[i]) < 0.1):
                        isWZA = True
